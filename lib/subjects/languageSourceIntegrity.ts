...
} from "../readings/structuredReading";
import type { SubjectKey } from "./subjectConfig";

const LANGUAGE_SUBJECT_KEYS = new Set<SubjectKey>([
  "english",
  "english-stage-8",
  "afrikaans",
  "afrikaans-stage-8",
...
const ENGLISH_SOURCE_LABEL_PATTERN =
...
const AFRIKAANS_EVIDENCE_PATTERN =
  /\b(uit die uittreksel|uit die teks|haal aan|aanhaling|teksbewyse|bewyse uit die teks|woorde? of frases?|taalgebruik|verwys na die teks|beeldspraak|toon|stemming|struktuur|vergelyk die tekste|identifiseer (?:twee )?voorbeelde?)\b/i;

const ENGLISH_ANALYTICAL_SKILL_PATTERN =
  /\b(language analysis|writer'?s (?:choices|language)|effect on the reader|narrative technique|narrative techniques|structural analysis|structure|tone|mood|imagery|figurative language|poetry|drama|characterisation|compare(?:\s+the)?\s+two\s+texts?|compare perspectives|evidence selection|quotation-based analysis|textual analysis|non-linear narrative|suspense)\b/i;
const AFRIKAANS_ANALYTICAL_SKILL_PATTERN =
  /\b(beeldspraak|taalgebruik|toon|stemming|struktuur|karakterisering|poesie|poësie|drama|verhaaltegnieke|teksanalise|bewyse|haal aan|uittreksel|vergelyk die tekste)\b/i;

const SOURCE_PREFIX_PATTERNS = {
  english:
    /^(Teaching Extract|Practice Extract|Extract|Passage|Poem|Dialogue|Story|Article|Speech|Letter|Text|Example)\s*:\s*(.*)$/i,
  afrikaans:
    /^(Voorbeeld|Uittreksel|Teks|Gedig|Dialoog|Drama-uittreksel|Verhaal)\s*:\s*(.*)$/i,
} as const;

export type ReadingSourceMaterialKind =
  | "teaching-prose"
  | "short-illustrative-example"
  | "substantial-source";

export type ReadingSourceSegment = {
  label: string | null;
  text: string;
  wordCount: number;
  candidateEvidenceCount: number;
  kind: ReadingSourceMaterialKind;
};

export type ReadingSourceMaterialClassification = {
  overallKind: ReadingSourceMaterialKind;
  segments: ReadingSourceSegment[];
  substantialSourceCount: number;
  shortExampleCount: number;
  teachingProseCount: number;
  totalCandidateEvidenceCount: number;
  supportsIndependentPractice: boolean;
};

export type SourceEvidenceQuestionCheckInput = {
...
export type SourceEvidenceSufficiencyResult = {
  ok: boolean;
  ...
  classification: ReadingSourceMaterialClassification;
};
...
function countCandidateEvidenceUnits(value: string) {
  return value
    .split(/(?<=[.!?])\s+|\n+/)
...
function blockToText(block: StructuredReadingBlock) {
  if (
    block.type === "heading" ||
...
function isAfrikaansSubjectKey(subjectKey: SubjectKey) {
  return subjectKey === "afrikaans" || subjectKey === "afrikaans-stage-8";
}

function isEnglishSubjectKey(subjectKey: SubjectKey) {
  return subjectKey === "english" || subjectKey === "english-stage-8";
}

function isSourceLabel(subjectKey: SubjectKey, label: string | null) {
...
function buildSegments(
  ...
  const segments: ReadingSourceSegment[] = [];
  ...
  for (const block of blocks) {
    ...
    const sourcePrefixMatch =
      block.type === "paragraph" ? blockText.match(sourcePrefixPattern) : null;
...
export function isLanguageSubjectKey(subjectKey: SubjectKey) {
  return LANGUAGE_SUBJECT_KEYS.has(subjectKey);
}

export function questionRequiresSourceEvidence({
  subjectKey,
  questionText,
  questionType,
}: SourceEvidenceQuestionCheckInput) {
...
export function classifyReadingSourceMaterial(
  readingContent: string,
  subjectKey: SubjectKey,
): ReadingSourceMaterialClassification {
  const segments = buildSegments(readingContentToBlocks(readingContent), subjectKey);
  const substantialSourceCount = segments.filter(
    (segment) => segment.kind === "substantial-source",
  ).length;
  const shortExampleCount = segments.filter(
    (segment) => segment.kind === "short-illustrative-example",
  ).length;
  const teachingProseCount = segments.filter(
    (segment) => segment.kind === "teaching-prose",
  ).length;
  ...
  return {
    ...
    supportsIndependentPractice:
      substantialSourceCount > 0 && totalCandidateEvidenceCount >= 2,
  };
}

export function lessonRequiresSubstantialSourceMaterial(args: {
  subjectKey: SubjectKey;
  readingTitle: string;
  instruction: string;
}) {
...
function requiresTwoTexts(question: string) {
...
function requiresTwoExamples(question: string) {
  return /\b(two|2)\b/i.test(question) || /\b(twee|2)\b/i.test(question);
}

function requiresBeginningAndEnding(question: string) {
  return /\b(beginning and ending|opening and ending|begin en einde)\b/i.test(question);
}

function requiresQuotedMaterial(question: string) {
  return /\b(quote|quotation|quoted|words? or phrases?|haal aan|aanhaling|woorde? of frases?)\b/i.test(question);
}

export function hasSufficientEvidenceForQuestion({
  subjectKey,
  questionText,
  questionType,
  guidance,
  readingContent,
}: SourceEvidenceQuestionCheckInput & { readingContent: string }): SourceEvidenceSufficiencyResult {
  ...
  if (
    requiresTwoTexts(normalizedQuestion) &&
  ...
  if (requiresBeginningAndEnding(normalizedQuestion)) {
    const structuralSegment = classification.segments.find(
      (segment) =>
        segment.kind === "substantial-source" &&
  ...
  if (
    requiresQuotedMaterial(normalizedQuestion) &&
...
export function buildLanguageReadingSourceIntegrityPrompt(args: {
  subjectKey: SubjectKey;
  readingTitle: string;
  instruction: string;
}) {
...
export function buildLanguageActivitySourceIntegrityPrompt(args: {
  subjectKey: SubjectKey;
  ...
  const sourceSummary =
    classification.overallKind === "substantial-source"
