...
test("audit marks teaching prose plus quote request as FAIL", () => {
  const result = auditLanguageActivity({
    subjectKey: "english",
  ...
  assert.equal(result.integrityStatus, "FAIL");
  assert.equal(result.questions[0].integrityStatus, "FAIL");
});

test("audit marks substantial English extract plus valid evidence question as OK or WARNING, not FAIL", () => {
  const result = auditLanguageActivity({
  ...
  assert.notEqual(result.integrityStatus, "FAIL");
});

test("audit marks one text plus compare the two texts as FAIL", () => {
  const result = auditLanguageActivity({
    subjectKey: "english",
    subjectLabel: "English 0861 - Stage 9",
    stageLabel: "Stage 9",
  ...
  assert.equal(result.integrityStatus, "FAIL");
  assert.match(result.questions[0].reason, /two substantial texts/i);
});

test("audit marks substantial Afrikaans extract plus valid haal aan question as OK or WARNING, not FAIL", () => {
  const result = auditLanguageActivity({
...
test("audit marks non-source-dependent knowledge question as OK", () => {
  const result = auditLanguageActivity({
    subjectKey: "english",
    subjectLabel: "English 0861 - Stage 9",
    stageLabel: "Stage 9",
  ...
  assert.equal(result.integrityStatus, "OK");
  assert.equal(result.questions[0].integrityStatus, "OK");
});

test("summary totals reflect OK, WARNING and FAIL counts", () => {
  const summary = summarizeLanguageAudit([
    {
      subjectKey: "english",
      subjectLabel: "English 0861 - Stage 9",
      stageLabel: "Stage 9",
  ...
  assert.equal(summary.totalActivitiesScanned, 3);
  assert.equal(summary.totalOk, 1);
  assert.equal(summary.totalWarning, 1);
