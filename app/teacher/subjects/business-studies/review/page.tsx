...
import {
  getBusinessStudiesActivityReviews,
  type TeacherActivityReview,
  type TeacherActivityReviewSubmission,
} from "@/lib/supabase/activityReviewReader";

const reviewHref = "/teacher/subjects/business-studies/review/submission";
...
function getFinalMark(
  ...
  return submission.finalMark === null
    ? "Pending"
    : `${submission.finalMark}/${totalMarks}`;
}
      ...
      {activity.submissions.map((submission) => (
        <div key={submission.id}>
          <div className="hidden grid-cols-[minmax(0,1.5fr)_0.8fr_1fr_1fr_auto] items-center gap-3 border-t border-orange-100 px-4 py-4 text-xs sm:grid">
            ...
            >
              {getFinalMark(submission, activity.totalMarks)}
            </span>
            <Link
              href={reviewHref}
...
export default async function BusinessStudiesActivityReviewPage() {
  let activities: TeacherActivityReview[] = [];
  let loadError = "";

  try {
    activities = await getBusinessStudiesActivityReviews();
  } catch (error) {
    console.error("Unable to load Business Studies activity reviews:", error);
    ...
    <main className="min-h-screen bg-slate-100 pb-24">
      <div className="mx-auto max-w-3xl px-4 pt-4">
        <div className="mb-5 rounded-[2rem] border border-orange-100 bg-white p-5 shadow-sm">
          ...
          <h1 className="text-3xl font-bold text-slate-900">Activity Review</h1>
          <p className="mt-1 text-sm text-slate-500">Business Studies Faculty</p>
        </div>
                ...
                <div className="space-y-4">
                  {group.activities.map((activity) => (
                    ...
                    >
                      ...
                      <SubmissionRows activity={activity} />
                    </details>
                  ))}
                </div>
              </section>
